...
use SilntiBundle\Model\Etudiant;
use SilntiBundle\Model\EtudiantQuery;
...
class EtudiantTableMap extends TableMap
{
     ...
     */
    const OM_CLASS = '\\SilntiBundle\\Model\\Etudiant';
     ...
     */
    const COL_ID_USER = 'etudiant.id_user';

    /**
     * the column name for the id_session field
     */
    const COL_ID_SESSION = 'etudiant.id_session';
    ...
    protected static $fieldNames = array (
        self::TYPE_PHPNAME       => array('IdUser', 'IdSession', ),
        self::TYPE_CAMELNAME     => array('idUser', 'idSession', ),
        self::TYPE_COLNAME       => array(EtudiantTableMap::COL_ID_USER, EtudiantTableMap::COL_ID_SESSION, ),
        self::TYPE_FIELDNAME     => array('id_user', 'id_session', ),
    ...
    protected static $fieldKeys = array (
        self::TYPE_PHPNAME       => array('IdUser' => 0, 'IdSession' => 1, ),
        self::TYPE_CAMELNAME     => array('idUser' => 0, 'idSession' => 1, ),
        self::TYPE_COLNAME       => array(EtudiantTableMap::COL_ID_USER => 0, EtudiantTableMap::COL_ID_SESSION => 1, ),
        self::TYPE_FIELDNAME     => array('id_user' => 0, 'id_session' => 1, ),
        self::TYPE_NUM           => array(0, 1, )
    );
  ...
  array (
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(EtudiantTableMap::COL_ID_USER);
            $criteria->addSelectColumn(EtudiantTableMap::COL_ID_SESSION);
        } else {
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(EtudiantTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(EtudiantTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new EtudiantTableMap());
        }
    }
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \SilntiBundle\Model\Etudiant) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(EtudiantTableMap::DATABASE_NAME);
            $criteria->add(EtudiantTableMap::COL_ID_USER, (array) $values, Criteria::IN);
        }
...
//
EtudiantTableMap::buildTableMap();
