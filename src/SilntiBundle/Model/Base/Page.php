...
abstract class Page implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\SilntiBundle\\Model\\Map\\PageTableMap';
    ...
    /**
     * The value for the titre_page field.
     *
     * @var        string
     */
    ...
    /**
     * Initializes internal state of SilntiBundle\Model\Base\Page object.
     */
    ...
    /**
     * Get the [titre_page] column value.
     *
     * @return string
     */
     ...
     * @return $this|\SilntiBundle\Model\Page The current object (for fluent API support)
     */
    public function setIdPage($v)
    {
        ...
        if ($this->id_page !== $v) {
            $this->id_page = $v;
            $this->modifiedColumns[PageTableMap::COL_ID_PAGE] = true;
        }
    ...
    /**
     * Set the value of [titre_page] column.
     *
     * @param string $v new value
     * @return $this|\SilntiBundle\Model\Page The current object (for fluent API support)
     */
    public function setTitrePage($v)
    {
        if ($v !== null) {
            $v = (string) $v;
        }

        if ($this->titre_page !== $v) {
            $this->titre_page = $v;
            $this->modifiedColumns[PageTableMap::COL_TITRE_PAGE] = true;
        }
    ...
    public function setIdUser($v)
    {
        ...
        if ($this->id_user !== $v) {
            $this->id_user = $v;
            $this->modifiedColumns[PageTableMap::COL_ID_USER] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : PageTableMap::translateFieldName('IdPage', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id_page = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : PageTableMap::translateFieldName('TitrePage', TableMap::TYPE_PHPNAME, $indexType)];
            $this->titre_page = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : PageTableMap::translateFieldName('IdUser', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\SilntiBundle\\Model\\Page'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(PageTableMap::COL_ID_PAGE)) {
            $modifiedColumns[':p' . $index++]  = 'id_page';
        }
        if ($this->isColumnModified(PageTableMap::COL_TITRE_PAGE)) {
            $modifiedColumns[':p' . $index++]  = 'titre_page';
        }
        if ($this->isColumnModified(PageTableMap::COL_ID_USER)) {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'id_page':
                        ...
                        break;
                    case 'titre_page':
                        $stmt->bindValue($identifier, $this->titre_page, PDO::PARAM_STR);
                        break;
                    case 'id_user':
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\SilntiBundle\Model\Page
     */
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[0], $arr)) {
            $this->setIdPage($arr[$keys[0]]);
        }
        if (array_key_exists($keys[1], $arr)) {
            $this->setTitrePage($arr[$keys[1]]);
        }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\SilntiBundle\Model\Page The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(PageTableMap::COL_ID_PAGE)) {
            $criteria->add(PageTableMap::COL_ID_PAGE, $this->id_page);
        }
        if ($this->isColumnModified(PageTableMap::COL_TITRE_PAGE)) {
            $criteria->add(PageTableMap::COL_TITRE_PAGE, $this->titre_page);
        }
        if ($this->isColumnModified(PageTableMap::COL_ID_USER)) {
            $criteria->add(PageTableMap::COL_ID_USER, $this->id_user);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \SilntiBundle\Model\Page (or compatible) type.
    ...
    {
        $copyObj->setTitrePage($this->getTitrePage());
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \SilntiBundle\Model\Page Clone of current object.
     * @throws PropelException
     */
    ...
    {
        if (null !== $this->aUser) {
            $this->aUser->removePage($this);
        }
        $this->id_page = null;
        $this->titre_page = null;
        $this->id_user = null;
        $this->alreadyInSave = false;
        $this->clearAllReferences();
        $this->resetModified();
